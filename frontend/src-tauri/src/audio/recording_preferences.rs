...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\protocolito-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("protocolito-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("protocolito-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/protocolito-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("protocolito-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/protocolito-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("protocolito-recordings")
    }
