...
export const MODEL_DISPLAY_CONFIG: Record<string, ModelDisplayInfo> = {
  'parakeet-tdt-0.6b-v3-int8': {
    ...
    icon: '⚡',
    tagline: 'Echtzeit • Maximale Geschwindigkeit, sehr gute Genauigkeit',
    recommended: true,
    tier: 'fastest'
  },
  'parakeet-tdt-0.6b-v2-int8': {
    ...
    icon: '📦',
    tagline: 'Echtzeit • Kleinere Größe',
    tier: 'balanced'
  },
  'parakeet-tdt-0.6b-v3-fp32': {
    friendlyName: 'Precise',
    icon: '🎯',
    tagline: '20× Echtzeit • Höhere Genauigkeit',
    tier: 'precise'
  }
};
...
export const PARAKEET_MODEL_CONFIGS: Record<string, Partial<ParakeetModelInfo>> = {
  'parakeet-tdt-0.6b-v3-int8': {
    description: 'Echtzeit auf M4 Max, auf Geschwindigkeit optimiert',
  ...
  'parakeet-tdt-0.6b-v2-int8': {
    description: '25× Echtzeit, kleinere Größe bei guter Genauigkeit',
  ...
  'parakeet-tdt-0.6b-v3-fp32': {
    description: '20× Echtzeit auf M4 Max, höhere Präzision',
...
export function getModelPerformanceBadge(quantization: QuantizationType): { label: string; color: string } {
  switch (quantization) {
    ...
    case 'Int8':
      return { label: 'Int8-quantisiert', color: 'green' };
    default:
