...
export function useModelConfiguration({ serverAddress }: UseModelConfigurationProps) {
  ...
  const handleSaveModelConfig = useCallback(async (updatedConfig?: ModelConfig) => {
    try {
      ...
      toast.error("Einstellungen konnten nicht gespeichert werden", { description: String(error) });
      if (error instanceof Error) {
        setError(error.message);
      } else {
        setError('Modellkonfiguration konnte nicht gespeichert werden: Unbekannter Fehler');
      }
    }
  }, [modelConfig]);
